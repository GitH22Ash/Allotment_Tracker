...
function AdminPanel() {
    ...
    const handleAssignGroups = async () => {
        ...
        try {
            ...
            // Refresh groups data if we're on the groups tab
            if (activeTab === 'groups') {
                fetchGroups();
            }
        } catch (err) {
    ...
    const handleManualAssign = async () => {
        ...
        try {
            setLoading(true);
            await axios.put(`${API_URL}/admin/groups/${selectedGroup}/assign`, {
    ...
    const handleUnassign = async (groupId) => {
        ...
        try {
            setLoading(true);
            await axios.put(`${API_URL}/admin/groups/${groupId}/unassign`);
        ...
        <div className="max-w-6xl mx-auto">
            ...
            <div className="flex space-x-1 mb-6 border-b">
                ...
                <TabButton 
                    ...
                    isActive={activeTab === 'supervisors'} 
            ...
            {activeTab === 'supervisors' && (
                <div className="bg-white p-6 rounded-lg shadow-lg">
                    ...
                    {loading ? (
                        <p className="text-center">Loading supervisors...</p>
                    ) : (
                                ...
                                <tbody className="bg-white divide-y divide-gray-200">
                                    {groups.map((group) => (
                                        <tr key={group.group_id}>
                                            ...
                                            <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                                                {group.supervisor_name || 'Unassigned'}
                                            </td>
                                            <td className="px-6 py-4 whitespace-nowrap">
                                                <span className={`px-2 inline-flex text-xs leading-5 font-semibold rounded-full ${
                                            ...
                                            <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                                {group.assigned_supervisor_id && (
                                                    <button
                                                        onClick={() => handleUnassign(group.group_id)}
                                ...
                                >
                                    ...
                                    {supervisors.filter(s => s.current_groups < s.max_groups).map((supervisor) => (
                                        <option key={supervisor.emp_id} value={supervisor.emp_id}>
                                            {supervisor.name} ({supervisor.current_groups}/{supervisor.max_groups} groups)
                                        </option>
                                    ))}
                                </select>
                            </div>
